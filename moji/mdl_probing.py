...
sys.path.append('../src')
from ScriptUtils import load_bias_in_bios_vectors, load_moji_vectors
...
def parse_args():
    ...
    parser.add_argument('--training_balanced', help='balancing of the training data', default="original")
...
def init_wandb(args):
    ...
    wandb.init(project="Moji DFL - MDL probing", config={
...
def main():
    ...
    task_name = f'moji_model_{args.model}_type_{args.type}_{args.testing_balanced}_training_{args.training_data}_{args.training_balanced}_seed_{args.seed}'
    run_MDL_probing(args, load_moji_vectors, task_name, shuffle=True)
